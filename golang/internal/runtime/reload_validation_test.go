...
func TestRuntimeReplacementValidatorRejectsProcessLifetimeChanges(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "Temporal API key file", field: "temporal.api_key_file", mutate: func(value *config.Config) { value.Temporal.APIKeyFile = "/var/run/secrets/llmtw/temporal-api-key-2" }},
