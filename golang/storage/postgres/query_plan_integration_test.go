...
func TestSpendSummaryQueryPlanUsesLedgerIndexes(t *testing.T) {
	...
	insertOperations := "INSERT INTO " + operations + " (operation_id, scope_id, operation_kind, api_version, operation_key_hmac, request_fingerprint_hmac, request_digest, request_schema_version, request_manifest_jsonb, request_inline_ciphertext, request_key_id, config_digest, state, result_inline_ciphertext, result_key_id, result_digest, result_byte_length, result_media_type, route_id, endpoint_id, provider, endpoint_family, resolved_model, operation_expires_at, reserved_cost_usd, incurred_cost_usd, actual_cost_usd, cost_status, cost_method, created_at, updated_at, completed_at, retention_expires_at) " +
		"SELECT md5('spend-plan-operation-'||i::text)::uuid, $1, 'generate', 'llm.temporal/v1', decode(md5('spend-plan-key-'||i::text),'hex'), decode(md5('spend-plan-fingerprint-'||i::text),'hex'), decode(md5('spend-plan-request-'||i::text),'hex'), 1, '{}'::jsonb, decode('01','hex'), 'query-plan-key', $2, 'completed', decode('02','hex'), 'query-plan-result-key', decode(md5('spend-plan-result-'||i::text),'hex'), 1, 'application/json', 'route-'||(i % 100)::text, 'endpoint-'||(i % 100)::text, 'provider-'||(i % 20)::text, 'chat', 'model-'||(i % 50)::text, $3 + interval '1 day', 1.0, 1.0, 1.0, 'exact', 'provider_reported', $3, $3, $3 + (i || ' microseconds')::interval, $3 + interval '1 day' FROM generate_series(1," + fmt.Sprint(rowCount) + ") AS series(i)"
	...
	insertExecutions := "INSERT INTO " + executions + " (query_execution_id, scope_id, api_version, operation_key_hmac, request_fingerprint_hmac, query_kind, request_jsonb, response_jsonb, response_digest, source, actual_cost_usd, cost_status, cost_method, started_at, completed_at, retention_expires_at) " +
		"SELECT md5('spend-plan-query-'||i::text)::uuid, $1, 'llm.temporal/query/v1', decode(md5('spend-plan-query-key-'||i::text),'hex'), decode(md5('spend-plan-query-fingerprint-'||i::text),'hex'), 'spend_summary', '{}'::jsonb, '{}'::jsonb, decode(md5('spend-plan-query-response-'||i::text),'hex'), 'worker', 0, 'exact', 'control_query_zero', $2, $2 + (i || ' microseconds')::interval, $2 + interval '1 day' FROM generate_series(1," + fmt.Sprint(rowCount) + ") AS series(i)"
